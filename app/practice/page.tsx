...
function VoiceInput() {
  ...
  function start() {
    ...
    rec.onresult = (e) => {
      ...
      setMemo(final + interim)
    }
    rec.onerror = (e) => {
      if (e.error === 'not-allowed')        setErrorMsg('マイクのアクセスが拒否されました')
      else if (e.error === 'audio-capture') setErrorMsg('マイクが見つかりません')
      else if (e.error !== 'no-speech')     setErrorMsg(`音声認識エラー: ${e.error}`)
    }
    rec.onend = () => {
      setListening(false)
      if (finalRef.current) setMemo(finalRef.current)
    }
    recRef.current = rec
    try { rec.start() } catch { setErrorMsg('音声認識を開始できませんでした') }
  }
    ...
    <div style={{ backgroundColor: CARD, borderRadius: '12px', boxShadow: '0 2px 12px rgba(28,66,48,0.07)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '18px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
          ...
          <span style={{ fontSize: '12px', fontWeight: 700, color: FOREST, letterSpacing: '0.06em' }}>音声メモ</span>
        </div>

        {/* Not supported — shown only after mount so SSR matches */}
        {mounted && !Ctor && (
        ...
        {listening && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
            <div style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: TERRACOTTA, animation: 'voicePulse 1s infinite', flexShrink: 0 }} />
        ...
        {/* Error */}
        {errorMsg && (
          <div style={{ fontSize: '12px', color: TERRACOTTA, marginBottom: '10px' }}>{errorMsg}</div>
        )}

        {/* Memo textarea — always visible */}
        <div style={{ fontSize: '12px', color: MUTED, marginBottom: '6px', fontWeight: 600 }}>
          メモ内容（各入力欄にコピーして使用できます）
        </div>
        <textarea
          value={memo}
          onChange={(e) => setMemo(e.target.value)}
          placeholder="マイクボタンを押して話すか、直接入力してください"
          style={{ width: '100%', minHeight: '88px', padding: '12px', borderRadius: '8px', border: `1.5px solid ${SAND_LIGHT}`, backgroundColor: CREAM, color: INK, fontSize: '13px', lineHeight: 1.65, resize: 'none', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
